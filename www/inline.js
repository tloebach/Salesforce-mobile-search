...
function onSuccessSfdcSearchRecordList(response) {
    ...
    $j.each(response, function(i, SearchResult) {
            ...
            var newLi = $j("<li class=details objtype=" + SearchResult.attributes.type + " id=" + SearchResult.Id + ">" + buildBody(SearchResult.Subject) + "<br><span class=content-hl-age>" + SearchResult.CaseNumber + "</span></li>");
            ul.append(newLi);
            } else {
            var newLi = $j("<li class=details objtype=" + SearchResult.attributes.type + " id=" + SearchResult.Id + ">" + buildBody(SearchResult.Name) + "</li>");
            ul.append(newLi);
            }
            });
...
function onSuccessSfdcSearchRecordListDetail(response) {
    ...
    if (response.attributes.type == "Contact") {
        ...
        htmlStr = "<div class=contact-div>" + "<div class=contact-title>Name</div>" + "<div class=contact-value>" + response.Name + "</div></div>" +
        "<div class=contact-div><div class=contact-title>Phone <a href=tel:" + phone + "></div><div class=contact-value>" + nullCheck(response.Phone) + "</a></div></div>" +
        "<div class=contact-div><div class=contact-title>Billing Address</div>" + "<div class=contact-value>" + nullCheck(response.BillingStreet)	+ "</div>" +
        "<div class=contact-value>" + nullCheck(response.BillingCity)	+ ", " + nullCheck(response.BillingState) +"</div>" +
        "<div class=contact-value>" + nullCheck(response.BillingCountry)	+ "</div>" +
        "</div></a>";
...
function buildDetNavbar(response) {
    var $j = jQuery.noConflict();
    ...
    var hasNav=false;
    // this is the only way I have found to create dynaic navbars. 
    var output = ['<div data-id="mainTab" data-role="navbar" data-position="fixed"><ul id="footer_tabs">'];
    
    
    // Save to Address book is broken in 1.8.1 cordova. It will start working in 2.2
    //    if (response.attributes.type == "Contact" || response.attributes.type == "Contact") {
    //      output.push('<li><a href="#" data-icon="phone" id="saveContact">Save</a></li>');
    //  }
    if (response.Id) {
        var linkStr = "'https://na1.salesforce.com/" + response.Id + "','_blank','location=yes'";
        output.push('<li><a href="#" data-icon="star" onclick=window.open(' + linkStr +  ')>Link</a></li>');
        hasNav=true;
    }
    if (response.MailingStreet) {
         var mapStr = "'http://maps.apple.com/maps?q=" + escape(lastResponse.MailingStreet) + escape(" " + lastResponse.MailingCity) + escape(" " + lastResponse.MailingState) + "'";        
        output.push('<li><a href="#" data-icon="star" onclick="window.open(' + mapStr + ')">Map</a></li>');
        hasNav=true;
    }
    if (response.Street) {
        var mapStr = "'http://maps.apple.com/maps?q=" + escape(lastResponse.Street) + escape(" " + lastResponse.City) + escape(" " + lastResponse.State) + "'";
        output.push('<li><a href="#" data-icon="star" onclick="window.open(' + mapStr + ')">Map</a></li>');
        hasNav=true;
    }
    if (response.BillingStreet) {
        var mapStr = "'http://maps.apple.com/maps?q=" + escape(lastResponse.BillingStreet) + escape(" " + lastResponse.BillingCity) + escape(" " + lastResponse.BillingState) + "'";
        output.push('<li><a href="#" data-icon="star" onclick="window.open(' + mapStr + ')">Map</a></li>');
        hasNav=true;
    }
    if (response.MobilePhone) {
        output.push('<li><a href="tel:' + mphone + '" data-icon="grid">Mobile</a></li>');
        hasNav=true;
    }
    if (response.Phone) {
        output.push('<li><a href="tel:' + phone + '" data-icon="grid">Phone</a></li>');
        hasNav=true;
    }
    if (response.Email) {
        output.push('<li><a href="mailto:' + response.Email + '" data-icon="star">Email</a></li>');
        hasNav=true;
    }
    output.push('</ul></div>');
    
    // This is the only way to get a dynamic navbar to render properly
    if (hasNav) {
     $j("#det-footer").html(output.join('')).trigger('create');
    }
...
function onSuccessSfdcSearchContent(response) {
    ...
    $j.each(response, function(i, SearchResult) {
            ...
            var newLi = $j("<li><a class=content-hl href=# onclick=window.open('" + forcetkClient.instanceUrl + "/sfc/servlet.shepherd/version/download/" + SearchResult.Id + "','_blank','location=yes')>" + "<span class=content-hl-span>" + buildBody(SearchResult.Title) + "</span> " + "<span class=content-hl-age>" + descStr + "<br>" + age + "</span>" + "<span class=content-hl-filetype>" + SearchResult.FileType + " "  + bytesToSize(SearchResult.ContentSize , 0) + "</span>" + "</a></li>");
            

            ul.append(newLi);
            
            } else if (SearchResult.attributes.type == "Document") {
            var newLi = $j("<li><a href=# onclick=window.open('" + forcetkClient.instanceUrl + "/servlet/servlet.FileDownload?file=" + SearchResult.Id + "','_blank','location=yes')>" + SearchResult.Name + "</a></li>");
            
            var newLi = $j("<li><a class=content-hl href=# onclick=window.open('" + forcetkClient.instanceUrl + "/servlet/servlet.FileDownload?file=" + SearchResult.Id + "','_blank','location=yes')>" + "<span class=content-hl-span>" + buildBody(SearchResult.Name) + "</span> " + "<span class=content-hl-age>" + descStr + "<br> " + age + "</span>" + "<span class=content-hl-filetype>" + SearchResult.Type + " " + bytesToSize(SearchResult.BodyLength , 0) + "</span>" + "</a></li>");
            ul.append(newLi);
            }
            });
...
function onSuccessSfdcChatterDetails2(response) {
    ...
    if(response.type ==  "ContentPost") {
         ...
         attStr = "<div class=renderImage><a href=# onclick=window.open('" + forcetkClient.instanceUrl + "/sfc/servlet.shepherd/version/download/" + response.attachment.versionId + "','_blank'," + "'location=yes')><img class=detail-img src=" + forcetkClient.instanceUrl + "/sfc/servlet.shepherd/version/renditionDownload?rendition=THUMB240BY180&versionId=" + response.attachment.versionId + "></a></div>";
         fileSize = bytesToSize(response.attachment.fileSize,0);

    } else if (response.type ==  "LinkPost"){
        attStr = "<div class=linkpost><a href=# onclick=window.open('" + response.attachment.url + "')>" + response.attachment.title + "</a></div>";
    }
    ...
    $j.each(response.comments.comments, function(i, comment) {
            ...
            attStr = "<div class=renderImage>" +
            "<a href=# onclick=window.open('" + forcetkClient.instanceUrl +
            "/sfc/servlet.shepherd/version/download/" + comment.attachment.versionId +
            "','_blank','location=yes')><img class=detail-img src=" + forcetkClient.instanceUrl + "/sfc/servlet.shepherd/version/renditionDownload?rendition=THUMB240BY180&versionId=" + comment.attachment.versionId + "></a></div>";
            ...
            attStr = "<div class=linkpost><a href=# onclick=window.open('" + comment.attachment.url + "')>" + comment.attachment.title + "</a></div>";
...
function onSuccessSfdcChatterDetails(response) {
    ...
    if(response.type ==  "ContentPost") {
      ...
      htmlStr = htmlStr + "<div class=renderImage><a href=# onclick=window.open('" + forcetkClient.instanceUrl + "/sfc/servlet.shepherd/version/download/" + response.attachment.versionId + "')><img src=" + forcetkClient.instanceUrl + "/sfc/servlet.shepherd/version/renditionDownload?rendition=THUMB240BY180&versionId=" + response.attachment.versionId + "></a></div>";
    } else if (response.type ==  "LinkPost"){
        htmlStr = htmlStr + "<div class=linkpost><a href=# onclick=window.open('" + response.attachment.url + "')>" + response.attachment.url + "</a></div>";
    }
...
function onListRecordTap() {
    ...
    if (objtype == "Case") {
        forcetkClient.search("FIND {" + getLastSearchTerm() + "} IN ALL FIELDS Returning " + objtype + "(Id, CaseNumber, Subject)", onSuccessSfdcSearchRecordList, onRecordErrorSfdc);
